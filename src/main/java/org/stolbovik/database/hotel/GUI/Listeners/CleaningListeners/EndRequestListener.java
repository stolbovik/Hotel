...
public class EndRequestListener implements ActionListener {

    private final JTextField requestField;
    private final JLabel info;
    private final RequestForCleaningController requestForCleaningController;

    public EndRequestListener(@NotNull JTextField requestField,
                                          @NotNull JLabel info) {
        this.info = info;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            requestForCleaningController.finishCleaningRoom(Integer.parseInt(requestField.getText()));
        } catch (Exception ex) {
            flag = false;
            info.setText(ex.getMessage());
        }
        if (flag) {
            info.setText("Уборка завершена");
        }
    }
